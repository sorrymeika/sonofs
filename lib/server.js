...
function heartbeat(cfg) {
    const hbCallback = () => {
        hbTimeout && clearTimeout(hbTimeout);
        hbTimeout = setTimeout(() => {
            hbTimeout = null;
            registerServer(cfg, hbCallback);
        }, 5000);
    };
    registerServer(cfg, hbCallback);
}

function registerServer({ groupId, serverId, registry }, cb) {
    const client = net.createConnection({
        ...
        // console.log(info);
        client.write(Buffer.from([groupId, serverId]));
    })
        .on('error', (err) => {
            cb && cb(err);
        })
...
function createMaster(cfg, callback) {
    ...
    const { serverId, root, groupId, rootId, registry } = cfg;
    ...
    function cb(err, fd) {
        ...
        const success = (cfgFd, filesIdxFd) => {
            ...
            heartbeat({ groupId, serverId, registry });
        };

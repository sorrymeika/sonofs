                ...
                if (success) {
                    fs.open(filePath, 'a', (err, fd) => {
                        ...
                        fs.fstat(fd, (err, stats) => {
                            ...
                            if (stats.size === 0) {
                                fs.write(fd, Buffer.from([0]), 0, 1, 1024 * 1024 * 1024, (err) => {
                                    if (err) {
                                        return reject(err);
                                    }
                                    resolve({ msg, fd });
    ...
    const server = net.createServer((socket) => {
        ...
        socket.on('data', (buf) => {
            ...
            if (type == 0) {
                ...
                const offset = buf.readUIntBE(6, 4);

                if (!filePromise) {
                    filePromise = getFileInfo(mime, fileSize);
                }
                const rangeSize = buf.length - 10;
                const headBuf = Buffer.from([0, 0, 0, 0, 0]);
                ...
                filePromise
                    .then(({ fd, msg }) => {
                        const { fileStart, fileName } = msg;
                        fs.write(fd, buf, 10, rangeSize, fileStart + offset, (err) => {
                            if (err) {
                                socket.write(headBuf);
                                return;
                            }

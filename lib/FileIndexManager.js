...
class FileIndexManager {
    ...
    async init() {
        ...
        this.logger.info('init - currentBundleId:', this.currentBundleId, 'currentBundleSize:', this.currentBundleSize, 'indexFileSize:', this.indexFileSize);
    }
    ...
    _loadIndex() {
        return new Promise((resolve, reject) => {
            fs.open(this.indexFilePath, fs.constants.O_WRONLY | fs.constants.O_CREAT, (err, indexFd) => {
                ...
                fs.stat(this.indexFilePath, (err, stats) => {
                    if (err) {
                        return reject(err);
                    }
